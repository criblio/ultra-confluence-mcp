...
describe('readFile', () => {
  ...
  it('refuses protected files (.env, lockfiles, .github)', async () => {
    // We don't even create these — the path check rejects before fs access.
    expect((await run<{ success: boolean }>(tools.readFile as Tool, { filePath: '.env' })).success).toBe(false);
    expect((await run<{ success: boolean }>(tools.readFile as Tool, { filePath: 'package-lock.json' })).success).toBe(false);
    expect((await run<{ success: boolean }>(tools.readFile as Tool, { filePath: '.github/workflows/foo.yml' })).success).toBe(false);
    expect((await run<{ success: boolean }>(tools.readFile as Tool, { filePath: 'scripts/agents/src/x.ts' })).success).toBe(false);
  });
...
describe('writeFile', () => {
  ...
  it('refuses to write into scripts/agents (self-modification)', async () => {
    const r = await run<{ success: boolean }>(tools.writeFile as Tool, {
      filePath: 'scripts/agents/src/evil.ts',
      content: '// pwned',
    });
    expect(r.success).toBe(false);
  });
  ...
  it('refuses to write a lockfile', async () => {
    expect(
      (await run<{ success: boolean }>(tools.writeFile as Tool, { filePath: 'package-lock.json', content: '{}' })).success
    ).toBe(false);
    expect(
      (await run<{ success: boolean }>(tools.writeFile as Tool, { filePath: 'yarn.lock', content: '' })).success
    ).toBe(false);
  });
});
