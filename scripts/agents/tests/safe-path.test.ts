...
import {
  ...
  isProtectedPath,
} from '../src/validation/safe-path.js';
...
describe('resolveSafePath', () => {
  ...
  it('rejects ../ even when it cancels out (defense in depth)', () => {
    // foo/../bar normalizes to bar — but the segment "../" appearing at all
    // is suspicious and we reject it.
...
describe('verifyResolvedRealpath', () => {
  ...
  it('rejects a symlink whose target is a protected path inside workingDir', async () => {
    // Create the protected target inside wd, then a symlink with an
    // innocent-looking name pointing at it.
    const protectedDir = path.join(wd, '.env');
    await fs.writeFile(protectedDir, 'SECRET=1', 'utf-8');
    const link = path.join(wd, 'config.txt');
    await fs.symlink(protectedDir, link);
